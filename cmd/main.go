...
import (
	...
	"fluid/agents/debian/internal/agent"
...
// enrollmentTokenForLog describes the enrollment secret without leaking it (length only).
...
func main() {
	...
	credentialsPath := flag.String("credentials", "/etc/fluid/debian/credentials.yaml", "Path to durable credentials YAML (organization_uuid + connection token)")
	enrollmentEnvPath := flag.String("enrollment-env", "/etc/fluid/debian/enrollment.env", "systemd EnvironmentFile path to remove after successful enrollment")
	flag.Parse()
	...
	if strings.TrimSpace(cfg.Controlplane.Token) == "" && enrollSecret != "" {
		...
		log.Printf("enrollment: starting exchange hostname=%q base_url=%s websocket_url=%s enrollment_token=%s",
			hostname,
			httpBase,
	...
	if err != nil {
		log.Fatalf("failed to initialize debian execution agent: %v", err)
	}

	if err := execAgent.Start(); err != nil {
		log.Fatalf("failed to start debian execution agent: %v", err)
	}
	log.Printf("debian agent process is running, waiting for signals")
